...
#Importamos la librería de pandas para análisis de datos
import pandas as pd
...
def _read_data(file_name):
    logger.info('1. Leyendo el archivo {}'.format(file_name))
...
def _add_newspaper_uid_column(df, newspaper_uid):
    logger.info('2. Llenando la columna newspaper_uid con {}'.format(newspaper_uid))
...
def _extract_host(df):
    logger.info('3. Extrayendo el host de las urls de los artículos')
...
def _fill_missing_titles(df):
    logger.info('4. Rellenando los títulos faltantes')
...
def _generate_uids_for_rows(df):
    logger.info('5. Generando los uids para cada fila')
...
def _remove_scape_characters_from_body(df):
    logger.info('6. Eliminando los caracteres de escape del body')
...
def _data_enrichment(df):
    logger.info('7. Enriqueciendo el df contando los tokens')
...
def _remove_duplicate_entries(df, column_name):
    logger.info('8. Eliminando entradas duplicadas')
...
def drop_rows_with_missing_values(df):
    logger.info('9. Eliminando entradas con valores faltantes')
    #Eliminando entradas duplicadas con base a una columna
    return df.dropna()
...
def _save_data_to_csv(df, filename):
    ...
    logger.info('10. Guardando el dataset limpio en el archivo: {}'.format(clean_filename))
